...
| `value`, `rank_probs`, `prior_before_noise` | **Fixed once the node exists.** Captured from a single `evaluate()` call at first expansion (or `null` if the node hasn't been expanded yet in this snapshot). A node created later (in a higher-N snapshot) has its own fixed value; an already-existing node's doesn't change. |
